...
export function UserAssetCard({ asset }: UserAssetCardProps) {
  return (
    <article className="flex flex-col rounded-xl bg-white p-4 shadow-[0_20px_40px_rgba(13,28,46,0.06)]">
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <h3 className="text-[1.75rem] leading-tight font-bold">{asset.name}</h3>
      ...
      <Link
        href={`/asset/${asset.id}`}
        className="w-full rounded-lg bg-gradient-to-br from-indigo-700 to-indigo-500 py-3 text-center text-sm font-bold text-white"
      >
        {asset.actionLabel}
      </Link>
    </article>
