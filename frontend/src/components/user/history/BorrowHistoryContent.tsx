...
export function BorrowHistoryContent({ history }: BorrowHistoryContentProps) {
  return (
    <>
      ...
      <div className="overflow-x-auto rounded-xl bg-white shadow-[0_20px_40px_rgba(13,28,46,0.06)] transition-colors duration-200 dark:bg-slate-900 dark:shadow-none">
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
          ...
          <tbody>
            {history.map((item) => (
              <tr key={item.id} className="border-b border-slate-100 dark:border-slate-700">
                ...
                <td className="px-6 py-6 text-sm text-slate-500 dark:text-slate-400">{item.borrowedAt}</td>
                <td className="px-6 py-6 text-sm text-slate-500 dark:text-slate-400">{item.returnedAt ?? "-"}</td>
                <td className="px-6 py-6 font-bold text-indigo-700">{currency(item.savingsUsd)}</td>
                <td className="px-8 py-6 text-right">
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase dark:bg-slate-800">
                    {item.status}
                  </span>
                </td>
              </tr>
